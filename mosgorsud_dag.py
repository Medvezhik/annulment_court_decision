...
def get_predict(text: str):
    request_url = "http://158.160.12.150/predict"
    response = requests.get(request_url, json={"text": text})
    if response.status_code != 200:
        print('server error', response.status_code)
        return float(0)

    print(response.json()["predict"])
    return float(response.json()["predict"])
...
def monitoring_metrics():
    ...
    data['preds'] = data['text'].apply(get_predict)
...
with DAG(dag_id='mosgorsud_weekly_dag', start_date=datetime.datetime(2022, 10, 20, 6, 0), end_date=datetime.datetime(2023, 10, 26, 6, 0), schedule="0 6 * * 6") as dag:
